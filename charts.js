...
function buildMetadata(sample) {
  d3.json("samples.json").then((data) => {
    ...
    var resultArray = metadata.filter(sampleObj => sampleObj.id == sample.id);
...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filterSample = data.samples.filter(sampleObj => sampleObj.id === sample.id);
    ...
    var Sampled = data.samples.sample_values

    // 7. Create the yticks for the bar chart.
    // Hint: Get the the top 10 otu_ids and map them in descending order  
    //  so the otu_ids with the most bacteria are last. 
    console.log(data.samples)
    var yticks = data.samples.otu_ids.slice(0, 10).sort((anElement, anotherElement) => anElement -
